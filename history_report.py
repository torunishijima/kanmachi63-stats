...
<style>
  * {{ box-sizing: border-box; }}
  ...
  .search-box {{
    ...
    font-size:.9em; background:#333; color:#eee; outline:none;
  }}
  ...
  .player-item.active {{ background:#FF6600; border-left-color:#FFFFCC; color:#fff; }}
  ...
  .detail-name {{ font-size:1.6em; font-weight:bold; color:#FF6600; }}
  .detail-meta {{ color:#888; font-size:.88em; margin-top:.3em; }}
  .detail-meta span {{ margin-right:1.5em; }}
  .detail-meta .inst {{ color:#FFFFCC; }}
  .detail-meta .days {{ color:#FF6600; font-weight:bold; }}

  .links {{ margin:.6em 0 1em; font-size:.85em; }}
  .links a {{ color:#FFFFCC; text-decoration:none; margin-right:1.2em; }}
  .links a:hover {{ text-decoration:underline; color:#FF6600; }}

  /* 年タブ */
  .yr-tabs {{ display:flex; flex-wrap:wrap; gap:.35em; margin:.2em 0 1.2em; }}
  .yr-tab {{
    border:1px solid #444; background:#222; padding:.3em .7em;
    border-radius:4px; cursor:pointer; font-size:.82em; color:#aaa;
  }}
  .yr-tab:hover {{ background:#333; border-color:#666; }}
  .yr-tab.active {{ background:#FF6600; color:#fff; border-color:#FF6600; font-weight:bold; }}
  ...
  .date-row {{
    ...
    padding:.5em 0; border-bottom:1px solid #333; font-size:.88em;
  }}
  .date-row:last-child {{ border-bottom:none; }}
  .date-label {{
    min-width:5.5em; color:#888; font-weight:bold; flex-shrink:0; padding-top:.1em;
  }}
  .co-chips {{ display:flex; flex-wrap:wrap; gap:.35em; }}
  .chip {{
    background:#2a2a2a; border-radius:4px; padding:.2em .6em;
    font-size:.85em; cursor:pointer; white-space:nowrap;
  }}
  .chip:hover {{ background:#FF6600; color:#fff; }}
  .chip .chip-inst {{ color:#888; font-size:.8em; margin-left:.3em; }}
  .no-co {{ color:#666; font-size:.85em; font-style:italic; }}
